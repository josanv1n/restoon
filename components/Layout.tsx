...
const Layout: React.FC<LayoutProps> = ({ children, activeRole, onRoleChange, onLogout }) => {
  ...
  return (
    <div className="min-h-screen flex bg-slate-950 text-slate-200 selection:bg-cyan-500 overflow-x-hidden">
      {/* Sidebar - Optimized for Mobile (w-16) and Tablet (w-20) */}
      <aside className="w-16 sm:w-20 md:w-64 glass border-r border-slate-800/50 flex flex-col z-50 sticky top-0 h-screen shrink-0">
        ...
        <nav className="flex-1 px-2 sm:px-4 py-4 sm:py-8 space-y-2 sm:space-y-3">
          {sidebarItems.map((item) => (
            ...
            >
              ...
              <span className="hidden md:block font-bold text-sm uppercase tracking-wider truncate">{item.label}</span>
